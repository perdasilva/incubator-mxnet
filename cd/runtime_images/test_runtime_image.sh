...
# Assumes the mxnet source directory is mounted on /mxnet and the cwd is /mxnet
...
if [[ $mxnet_variant == *mkl ]]; then
    PYTHONPATH=./python/ python3 tests/python/mkl/test_mkldnn.py
fi

PYTHONPATH=./python/ python3 tests/python/train/test_conv.py ${test_conv_params}
PYTHONPATH=./python/ python3 example/image-classification/train_mnist.py ${mnist_params}
