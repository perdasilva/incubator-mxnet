...
def docker_run(platform, function_name, use_nvidia, shared_mem = '500m', env_vars = '', runtime_functions_path = '/work/runtime_functions.sh') {
  def command = "ci/build.py %ENV_VARS% --docker-registry ${env.DOCKER_CACHE_REGISTRY} %USE_NVIDIA% --platform %PLATFORM% --docker-build-retries 3 --shm-size %SHARED_MEM% %RUNTIME_FUNCTIONS_PATH% %FUNCTION_NAME%"
  ...
  command = command.replaceAll('%RUNTIME_FUNCTIONS_PATH%', runtime_functions_path)
